...
public protocol FederatedServiceRouter: Sendable {
    ...
    var callbackCompletion: ((Request, String) async throws -> ResponseEncodable) { get }
...
extension FederatedServiceRouter {
    ...
    public func configureRoutes(
        ...
        router.get(authURL.pathComponents) { req async throws -> Response in
            let redirect: Response = req.redirect(to: try await self.authURL(req))
            if let authenticateCallback = authenticateCallback {
    ...
    public func fetchToken(from request: Request) async throws -> String {
        ...
        let buffer = try await body.encodeResponse(for: request)
    ...
    public func callback(_ request: Request) async throws -> Response {
        ...
        return try await response.encodeResponse(for: request).get()
    }
}
