...
public protocol FederatedServiceTokens {
   ...
   /// The name of the environment variable that has the issuer URL.
   static var issuingUrlEnvKey: String { get }

   /// The client secret for the OAuth provider that the service is connected to.
   var issuingUrl: String { get }

   /// Gets the client ID and secret from the environment variables and store them.
   init() throws
}

extension FederatedServiceTokens {
   public var wellknown: String {
      "\(issuingUrl)/.well-known/openid-configuration"
   }
}
